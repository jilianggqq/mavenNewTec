...
public class ElevatorController implements IElevatorController, ElevatorEventListener {
	...
	public void status() {
		...
		for (Elevator el : elevators) {
			System.out.println(" elID =  " + i++ + " CurrentFloor = " + el.getCurrentFloor() + " Moving =  " + el.getMove() + " DIRECTION = "
					+ el.getDirection() + "\n");
		}
	}
	...
	public void pickup(int floor, boolean direction) {
		if ((floor < 0) || (floor >= numFloors))
			throw new IllegalArgumentException();
		// shufling the order of elevators in the case where most of the elevators are
		// on the same floors and they are picked up at the same time from different floors
...
//		StdRandom.shuffle(elevatorIDs);
		...
		for (int elID : elevatorIDs) {
			...
			if ((elevator.getMove() == false)
					|| ((userDirection == Elevator.DIRECTION.UP) && (elevator.getDirection() == Elevator.DIRECTION.UP)
							&& (floor >= elevator.getCurrentFloor()))
					|| ((userDirection == Elevator.DIRECTION.DOWN) && (elevator.getDirection() == Elevator.DIRECTION.DOWN)
							&& (floor <= elevator.getCurrentFloor())))
